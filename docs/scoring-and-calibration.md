...
The calibration script (`calibrate-fast`) uses three API calls per repo:

1. **GraphQL batch query** — fetches all metrics for 5 repos in a single aliased query, minimizing round trips. Only the fields needed for the metrics above are requested — no over-fetching.
...
The GitHub Search API index can be stale and does not support all necessary exclusion criteria. These filters are applied in code after results are returned.
...
| Excluded languages: `Jupyter Notebook`, `Adblock Filter List`, `TeX`, `YAML` | These indicate notebook collections, filter lists, document repos, or config/data repos — not software projects with a meaningful PR and issue workflow |
...
| Security bypass tools | `otp bypass`, `2fa bypass`, `account switcher`, `account manager` | Credential tools and account-cycling repos have atypical activity patterns that would distort calibration |
| Collection/resource repos | `awesome`, `curated list`, `cheatsheet`, `roadmap`, `interview questions`, `study plan`, `learning path`, `public apis`, `system design primer` | These repos attract stars through content curation or virality, not software quality — their activity patterns are not representative of software projects |
...
**Expected runtime:** ~30–60 minutes with a single token; under 20 minutes with 3 tokens.
...
4. Review `lib/scoring/calibration-data.json` — verify sample sizes are ≥ 50 per bracket and percentile values are plausible
5. Commit with a message such as: `chore: recalibrate scoring thresholds (2026-Q2, N=207 repos)`
6. The UI staleness warning clears automatically once the new `generated` date is within 6 months
